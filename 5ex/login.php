...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if ($errors['usr_pass']) {
   ...
   $values = array();
   $values['usr_login'] = empty($_COOKIE['usr_login']) ? '' : $_COOKIE['usr_login'];
   $values['usr_pass'] = empty($_COOKIE['usr_pass']) ? '' : $_COOKIE['usr_pass'];
...
<body>
    ...
    <div class="content">
        ...
        <form action="" method="POST">
            ...
            <label>
                Пароль:<br>
                <input name="usr_pass" <?php if ($errors['usr_pass']) { print 'class="error"'; } ?> ?> /><br>
...
<?php
  ...
  foreach($checkPass as $user)
      $tempPass=$user['usr_pass'];
  if ($tempPass != $usr_pass){
